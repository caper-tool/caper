module Caper.PermissionsI2 (permCheckBigInt, permCheckTree) where
import Data.List
import Data.Maybe
...
class Monad m => PermMonad m where
        runPermMonad :: m Bool -> Bool
        splitAny :: m Bool -> m Bool
        splitAll :: m Bool -> m Bool
        isEmptyInter :: [(Int,Bool)] -> m Bool
...
instance (PermModel p) => PermMonad ((->) p) where
        runPermMonad a = a emptyModel
        isEmptyInter = flip isEmptyIntersection
        splitAny a m = 
